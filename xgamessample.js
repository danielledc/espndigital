...
var tuneInMessage = {
  WeekBefore: {
    timeStamp: 1453525200000,
    date: 23,
    showMessage:
      '<span id="smallerMessage"><span id="small">Starts</span> Friday 7<span id="timeZone">PM/ET</span></span>'
  },
  DayBefore: {
    timeStamp: 1453957200000,
    date: 28,
    showMessage:
      '<span id="smallerMessage"><span id="small">Starts</span> Tomorrow 7<span id="timeZone">PM/ET</span></span>'
  },
  Day1Show: {
    BeforeShow: {
      timeStamp: 1454043600000,
      date: 29,
      showMessage:
        '<span id="smallerMessage"><span id="small">Starts</span> Tonight 7<span id="timeZone">PM/ET</span></span>'
    },
    DuringShow: { timeStamp: 1454112000000, showMessage: "LIVE NOW" },
    AfterShow: {
      timeStamp: 1454126400000,
      showMessage:
        '<span id="smallerMessage"><span id="small">Continues</span> Tomorrow 1<span id="timeZone">PM/ET</span></span>'
    }
  },
  Day2Show: {
    BeforeShow1: {
      timeStamp: 1454130000000,
      date: 30,
      showMessage:
        '<span id="smallerMessage"><span id="small">Continues</span> Today 1<span id="timeZone">PM/ET</span></span>'
    },
    DuringShow1: { timeStamp: 1454176800000, showMessage: "LIVE NOW" },
    BeforeShow2: {
      timeStamp: 1454194800000,
      showMessage:
        '<span id="smallerMessage"><span id="small">Continues</span> Tonight 9<span id="timeZone">PM/ET</span></span>'
    },
    DuringShow2: { timeStamp: 1454205600000, showMessage: "LIVE NOW" },
    AfterShow: {
      timeStamp: 1454212800000,
      showMessage:
        '<span id="smallerMessage"><span id="small">Continues</span> Tomorrow 12<span id="timeZone">PM/ET</span></span>'
    }
  },
  Day3Show: {
    BeforeShow: {
      timeStamp: 1454216400000,
      date: 31,
      showMessage:
        '<span id="smallerMessage"><span id="small">Continues</span> Today 12<span id="timeZone">PM/ET</span></span>'
    },
...
function changeMessage(size) {
  ...
  if (
    nowTimestamp >= tuneInMessage["WeekBefore"].timeStamp &&
    nowTimestamp < tuneInMessage["DayBefore"].timeStamp
  ) {
    if (tuneInMessage["WeekBefore"].date > now.getDate()) {
      ...
      document.getElementById("showScheduleMessage").innerHTML =
        tuneInMessage["WeekBefore"].showMessage;
      document.getElementById("abcLogo").style.display = "none";
      document.getElementById("getReady").style.display = "inline-block";
    }
  } else if (
    nowTimestamp >= tuneInMessage["DayBefore"].timeStamp &&
    nowTimestamp < tuneInMessage["Day1Show"].BeforeShow.timeStamp
  ) {
    document.getElementById("abcLogo").style.display = "none";
    if (tuneInMessage["DayBefore"].date > now.getDate()) {
      ...
      document.getElementById("showScheduleMessage").innerHTML =
        tuneInMessage["DayBefore"].showMessage;
      document.getElementById("seeSchedule").style.display = "inline-block";
    }
  } else if (
    nowTimestamp >= tuneInMessage["Day1Show"].BeforeShow.timeStamp &&
    nowTimestamp < tuneInMessage["Day1Show"].DuringShow.timeStamp
  ) {
    document.getElementById("abcLogo").style.display = "none";
    if (tuneInMessage["Day1Show"].BeforeShow.date > now.getDate()) {
      ...
      document.getElementById("showScheduleMessage").innerHTML =
        tuneInMessage["Day1Show"].BeforeShow.showMessage;
    ...
    nowTimestamp >= tuneInMessage["Day1Show"].DuringShow.timeStamp &&
    nowTimestamp < tuneInMessage["Day1Show"].AfterShow.timeStamp
  ) {
    document.getElementById("showScheduleMessage").innerHTML =
      tuneInMessage["Day1Show"].DuringShow.showMessage;
    document.getElementById("xgWrap").onclick = function() {
    ...
    nowTimestamp >= tuneInMessage["Day1Show"].AfterShow.timeStamp &&
    nowTimestamp < tuneInMessage["Day2Show"].BeforeShow1.timeStamp
  ) {
    document.getElementById("showScheduleMessage").innerHTML =
      tuneInMessage["Day1Show"].AfterShow.showMessage;
    document.getElementById("seeSchedule").style.display = "inline-block";
    document.getElementById("xgWrap").onclick = function() {
      setClickThrough(clickThroughs["afterShow"]);
    };
    ...
    nowTimestamp >= tuneInMessage["Day2Show"].BeforeShow1.timeStamp &&
    nowTimestamp < tuneInMessage["Day2Show"].DuringShow1.timeStamp
  ) {
    document.getElementById("xgWrap").onclick = function() {
      setClickThrough(clickThroughs["afterShow"]);
    };
    if (tuneInMessage["Day2Show"].BeforeShow1.date > now.getDate()) {
      ...
      document.getElementById("showScheduleMessage").innerHTML =
        tuneInMessage["Day2Show"].BeforeShow1.showMessage;
      document.getElementById("espnLogo").style.display = "none";
      document.getElementById("seeSchedule").style.display = "inline-block";
    }
  } else if (
    nowTimestamp >= tuneInMessage["Day2Show"].DuringShow1.timeStamp &&
    nowTimestamp < tuneInMessage["Day2Show"].BeforeShow2.timeStamp
  ) {
    document.getElementById("showScheduleMessage").innerHTML =
      tuneInMessage["Day2Show"].DuringShow1.showMessage;
    document.getElementById("xgWrap").onclick = function() {
    ...
    nowTimestamp >= tuneInMessage["Day2Show"].BeforeShow2.timeStamp &&
    nowTimestamp < tuneInMessage["Day2Show"].DuringShow2.timeStamp
  ) {
    document.getElementById("xgWrap").onclick = function() {
      setClickThrough(clickThroughs["afterShow"]);
    };
    document.getElementById("showScheduleMessage").innerHTML =
      tuneInMessage["Day2Show"].BeforeShow2.showMessage;
    document.getElementById("abcLogo").style.display = "none";
    document.getElementById("seeSchedule").style.display = "inline-block";
  } else if (
    nowTimestamp >= tuneInMessage["Day2Show"].DuringShow2.timeStamp &&
    nowTimestamp < tuneInMessage["Day2Show"].AfterShow.timeStamp
  ) {
    document.getElementById("showScheduleMessage").innerHTML =
      tuneInMessage["Day2Show"].DuringShow2.showMessage;
    document.getElementById("xgWrap").onclick = function() {
    ...
    nowTimestamp >= tuneInMessage["Day2Show"].AfterShow.timeStamp &&
    nowTimestamp < tuneInMessage["Day3Show"].BeforeShow.timeStamp
  ) {
    document.getElementById("showScheduleMessage").innerHTML =
      tuneInMessage["Day2Show"].AfterShow.showMessage;
    document.getElementById("xgWrap").onclick = function() {
      setClickThrough(clickThroughs["afterShow"]);
    };
    document.getElementById("abcLogo").style.display = "none";
    ...
    nowTimestamp >= tuneInMessage["Day3Show"].BeforeShow.timeStamp &&
    nowTimestamp < tuneInMessage["Day3Show"].DuringShow.timeStamp
  ) {
    document.getElementById("abcLogo").style.display = "none";
    document.getElementById("seeSchedule").style.display = "inline-block";
    document.getElementById("xgWrap").onclick = function() {
    ...
    if (tuneInMessage["Day3Show"].BeforeShow.date > now.getDate()) {
      ...
      document.getElementById("showScheduleMessage").innerHTML =
        tuneInMessage["Day3Show"].BeforeShow.showMessage;
    }
  } else if (nowTimestamp >= tuneInMessage["Day3Show"].DuringShow.timeStamp) {
    document.getElementById("abcLogo").style.display = "none";
    document.getElementById("showScheduleMessage").innerHTML =
      tuneInMessage["Day3Show"].DuringShow.showMessage;
    document.getElementById("live").style.display = "inline-block";
    document.getElementById("xgWrap").onclick = function() {
  ...
  if (size == "320x50") {
    if (
      document
        .getElementById("showScheduleMessage")
        .innerHTML.indexOf("Continues") > -1 ||
      document
        .getElementById("showScheduleMessage")
        .innerHTML.indexOf("Starts") > -1
    ) {
      if (
        document
          .getElementById("showScheduleMessage")
          .innerHTML.indexOf("Friday") == -1
      ) {
        document.getElementById("showScheduleMessage").innerHTML =
          "All Weekend Long<br/>";
    ...
    if (
      document.getElementById("showScheduleMessage").innerHTML.indexOf("LIVE") >
      -1
    ) {
  ...
  if (size == "300x250" || size == "728x90") {
    if (
      document
        .getElementById("showScheduleMessage")
        .innerHTML.indexOf("LIVE") == -1 &&
      document
        .getElementById("showScheduleMessage")
        .innerHTML.indexOf("29-31") == -1
    ) {
      if (size == "300x250") {
    ...
    document.getElementById(
      ...
      .getElementById("showScheduleMessage")
      .innerHTML.replace("Starts", "Starts<br/>");
    document.getElementById(
      "showScheduleMessage"
    ).innerHTML = document
      .getElementById("showScheduleMessage")
      .innerHTML.replace("Continues", "Continues<br/>");
    if (size == "728x90") {
      if (
        document
          .getElementById("showScheduleMessage")
...
function randomizeSponsor(size) {
  ...
  if (randomNumber >= 0 && randomNumber <= 0.11)
    ...
    document.getElementById("sponsor").innerHTML = document
      .getElementById("sponsor")
      .innerHTML.replace("SPONSORED BY", "BROUGHT TO YOU BY");
    if (size == "1280x100") {
